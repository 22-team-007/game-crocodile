...
export default class Oauth extends ApiBase implements OauthAPIType {
  public async signIn (code:string,redirect_uri:string) {
    const r = await this.POST('/api/v2/oauth/yandex', { body: JSON.stringify({ code, redirect_uri })});
    return await r.text();
  }

  public async setvice (redirect_uri:string):Promise<string> {
    const r = await this.GET(`/api/v2/oauth/yandex/service-id?redirect_uri=${encodeURI(redirect_uri)}`);
